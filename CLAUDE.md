...
- **Recommendations**: Curiosity engine screen — browse detected knowledge gaps (chip list with severity colors), scan wiki for gap-filling documents, view scored recommendations with predicted new edges
...
1. **One-tap ingest** — Add ingest button to recommendation cards. The "Want me to ingest it?" moment from the origin story. Wire `_RecommendationCard` → ingest flow → post-ingest evaluation.
2. **Post-ingest evaluation feedback loop** — After ingesting a recommended document, run `evaluateRecommendation()` to compare predicted edges vs actual. Display accuracy on the recommendation card. Feed accuracy back into future recommendation prompts (follows FSRS Phase 4c calibration pattern).
3. **Autonomous gap monitoring** — Watch `gapAnalysisProvider` for new/worsening gaps and surface proactive notifications. The system notices gaps without being asked.
4. **Beyond-wiki search** — When Outline wiki doesn't have content that bridges a gap, search the broader internet. Recommend external resources (papers, tutorials, courses) with the same scoring pipeline.
5. **Autonomous ingestion** — With user consent, the system finds, fetches, and ingests gap-filling content automatically. The curiosity engine becomes self-feeding.
